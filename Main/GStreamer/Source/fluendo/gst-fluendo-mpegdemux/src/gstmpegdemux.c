...
gst_flups_demux_src_query (GstPad * pad, GstQuery * query)
{
  ...
  switch (GST_QUERY_TYPE (query)) {
    ...
    case GST_QUERY_DURATION:
    {
      ...
      if (duration > demux->cached_duration)
        diff = duration - demux->cached_duration;
      else
        diff = demux->cached_duration - duration;
      
      if (diff > (GST_SECOND << 1))
        demux->cached_duration = duration;
      else
        duration = demux->cached_duration;

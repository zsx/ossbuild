...
#define is_errno(_err)      (WSAGetLastError() == (_err))
#ifndef EINPROGRESS
...
#define is_errno(_err)      (errno == (_err))
...
#ifdef BEOS
  if (setsockopt(s, SOL_SOCKET, SO_NONBLOCK, &on, sizeof(on)) == -1) {
    return -1;
  }
#elif defined WIN32
...
/*     _x_message(stream, XINE_MSG_CONNECTION_REFUSED, "can't put socket in non-blocking mode", strerror(errno), NULL); */
  ...
  for (i = 0; h->h_addr_list[i]; i++) {
    ...
    if (connect(s, (struct sockaddr *)&sin, sizeof(sin)) ==-1 && is_errno(EINPROGRESS)) {
      /* FIXME: de-xine-ification */
...
static int mmsh_internal_winsock_load(void)
{
  if (iWinsockNeed) {
